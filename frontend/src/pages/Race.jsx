...
const SignupDialog = ({ raceId }) => {
  ...
  const handleSignup = () => {
    const url = tipo === "corredor" ? RUNNERSIGNUPS_URL : VOLUNTEERSIGNUPS_URL;
    const body = tipo === "corredor"
      ? { race: raceId, state: "PENDENTE" }
      : { race: raceId, role: selectedRole, state: "PENDENTE" };

    if (tipo === "voluntario" && !selectedRole) return;
